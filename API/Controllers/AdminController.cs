...
namespace API.Controllers
{
    public class AdminController : BaseApiController
    {
        ...
        public AdminController(UserManager<AppUser> userManager, UnitOfWork uow, PhotoService photoService)
        {
        ...
        public async Task<ActionResult> EditRoles(string username, [FromQuery]string roles)
        {
            ...
            if (!result.Succeeded) return BadRequest("Failed adding a role!");

            result = await _userManager.RemoveFromRolesAsync(user, userRoles.Except(selectedRoles));

            if (!result.Succeeded) return BadRequest("Failed removing a role!");
